...
hist = model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=nb_epoch, batch_size=batch_size, callbacks=[early_stopping, reduce_lr])
plot_history(hist)
...
sns.heatmap(cm, annot=True, xticklabels=genre_encoder.get_feature_names_out(), yticklabels=genre_encoder.get_feature_names_out())
