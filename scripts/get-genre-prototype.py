...
def get_band_genre(band_name):
    ...
    while genres is None:
        ...
        try:
            ...
            print(f"Erste Suche nach P136 auf der {i}ten Seite zu {band_name} fehlgeschlagen")
            try:
                page = next(pywikibot.pagegenerators.SearchPageGenerator(band_name, site=site))
                item = pywikibot.ItemPage.fromPage(page)
...
#                break  # Schleife beenden, wenn alle Artikel durchsucht wurden
    ...
    if genres is None:
        for result in pywikibot.pagegenerators.SearchPageGenerator(band_name, site=site):
